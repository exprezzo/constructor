var EdicionEventos = function(){
	...
	this.setDSReporte = function(){		
		...
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/eventos/buscarReporte',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="fk_reporte"]').wijcombobox('option','data',datasource);
	};
		
	
	this.borrar=function(){		
		var r=confirm("¿Eliminar Evento ?");
		if (r==true){
		  this.eliminar();
		}
	}
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Evento: ' + getValorCampo('descripcion'));
		}else{
	...
	this.guardar=function(){
		...
		if (selectedIndex == -1){
			paramObj['fk_reporte'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_reporte'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_reporte'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
