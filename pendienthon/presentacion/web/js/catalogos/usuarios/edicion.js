var EdicionUsuarios = function(){
	...
	this.configurarComboRol=function(){
		var me=this;
		...
		$('select[name="fk_rol"]').wijcombobox({			
		 ...
		 $('.contenedor_fk_rol input[role="textbox"]').bind('focus', function(){			
			if (me.rolEnAjax) return true;			
			me.setDSRol();
			me.rolEnAjax=true;
		 });
	};
	...
	this.configurarComboEmpresa=function(){
		var me=this;
		...
		$('select[name="fk_empresa"]').wijcombobox({			
		 ...
		 $('.contenedor_fk_empresa input[role="textbox"]').bind('focus', function(){			
			if (me.empresaEnAjax) return true;			
			me.setDSEmpresa();
			me.empresaEnAjax=true;
		 });
	};
		
		
	this.setDSEmpresa = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/usuarios/buscarEmpresa',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
...
this.configurarComboRol();
this.configurarComboEmpresa();
	};
