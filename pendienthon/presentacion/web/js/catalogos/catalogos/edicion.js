var EdicionCatalogos = function(){
	...
	this.configurarComboModulo=function(){
		var me=this;
		...
		$('select[name="fk_modulo"]').wijcombobox({			
		 ...
		 $('.contenedor_fk_modulo input[role="textbox"]').bind('focus', function(){			
			if (me.moduloEnAjax) return true;			
			me.setDSModulo();
			me.moduloEnAjax=true;
		 });
	};
		
		
	this.setDSModulo = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/catalogos/buscarModulo',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
	...
	this.configurarComboTabla=function(){
		var me=this;
		...
		$('select[name="tabla"]').wijcombobox({			
		 ...
		 $('.contenedor_tabla input[role="textbox"]').bind('focus', function(){			
			if (me.tablaEnAjax) return true;			
			me.setDSTabla();
			me.tablaEnAjax=true;
		 });
	};
	...
	this.setDSTabla = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/catalogos/buscarTabla',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="tabla"]').wijcombobox('option','data',datasource);
	};
	...
	this.guardar=function(){
		...
		//-----------------------------------		
		var selectedIndex = $('[name="tabla"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="tabla"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['tabla'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['tabla'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['tabla'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
...
this.configurarComboModulo();
this.configurarComboTabla();
	};
