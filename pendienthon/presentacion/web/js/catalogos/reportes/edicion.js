var EdicionReportes = function(){
	this.editado=false;
	this.tituloNuevo='Nuevo Reporte';
	...
	this.setDSFk_autor = function(){		
		...
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/reportes/buscarUsuario',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
	...
	this.configurarComboFk_solicitante=function(){
		var me=this;
		
		$('select[name="fk_solicitante"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:false,			
		 ...
		 $('.contenedor_fk_solicitante input[role="textbox"]').bind('focus', function(){			
			if (me.Fk_solicitanteEnAjax) return true;			
			me.setDSFk_solicitante();
			me.Fk_solicitanteEnAjax=true;
		 });
	};
		
		
	this.setDSFk_solicitante = function(){		
		...
		$('select[name="fk_solicitante"]').wijcombobox('option','data',datasource);
	};
	...
	this.configurarComboFk_empresa=function(){
		...
		$('select[name="fk_empresa"]').wijcombobox({			
		 ...
		 $('.contenedor_fk_empresa input[role="textbox"]').bind('focus', function(){			
	...
	this.setDSFk_empresa = function(){		
		...
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/reportes/buscarEmpresa',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
	...
	this.borrar=function(){		
		var r=confirm("¿Eliminar Reporte?");
		if (r==true){
		  this.eliminar();
		}
	}
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Reporte: ' + getValorCampo('num_reporte'));
		}else{
	...
	this.guardar=function(){
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_solicitante"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_solicitante"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_solicitante'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_solicitante'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_solicitante'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
		...
		//-----------------------------------
		var datos=paramObj;
		$(tabId+' .tabla_eventos').wijgrid('endEdit');
				var eventos=$(tabId+' .tabla_eventos').wijgrid('data');
				datos.eventosDeReporte = eventos;
		...
		$.ajax({
			...
			if ( resp.success == true	){
				...
				//--------------------
				var elementos=resp.datos.eventosDeReporte;	

				var grid=$(me.tabId+" .tabla_eventos");
...
this.configurarComboFk_solicitante();
