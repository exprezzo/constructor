...
class CatalogoModelo extends Modelo{	
	...
	var $pk='id';
	var $campos= array('id', 'fk_modulo', 'nombre_Modulo', 'nombre', 'controlador', 'modelo', 'tabla', 'nombre_tabla', 'pk_tabla', 'icono', 'titulo_nuevo', 'titulo_edicion', 'titulo_busqueda', 'msg_creado', 'msg_actualizado', 'pregunta_eliminar', 'msg_eliminado', 'msg_cambios', 'campos_busqueda');
	...
	function buscar($params){
		...
		if ( !empty($params['filtros']) ){
			foreach($params['filtros'] as $filtro){
				...
				if ( $filtro['dataKey']=='nombre_Modulo' ) {
					$filtros .= ' Modulo0.nombre like :nombre_Modulo OR ';
				} 
				...
				if ( $filtro['dataKey']=='nombre_tabla' ) {
					$filtros .= ' tabla1.nombre like :nombre_tabla OR ';
				} 
		...
		$joins='
 LEFT JOIN system_modulos AS Modulo0 ON Modulo0.id = Catalogo.fk_modulo
 LEFT JOIN constructor_tabla AS tabla1 ON tabla1.id = Catalogo.tabla';
		...
		if ( !empty($params['filtros']) ){
			...
			if ( $filtro['dataKey']=='nombre_Modulo' ) {
				$sth->bindValue(':nombre_Modulo', '%'.$filtro['filterValue'].'%', PDO::PARAM_STR );
			}
			if ( $filtro['dataKey']=='nombre' ) {
			...
			if ( $filtro['dataKey']=='nombre_tabla' ) {
				$sth->bindValue(':nombre_tabla', '%'.$filtro['filterValue'].'%', PDO::PARAM_STR );
			}
			if ( $filtro['dataKey']=='pk_tabla' ) {
		...
		if ($paginar){
			$limit=$params['limit'];
			$start=$params['start'];
			$sql = 'SELECT Catalogo.id, Catalogo.fk_modulo, Modulo0.nombre AS nombre_Modulo, Catalogo.nombre, Catalogo.controlador, Catalogo.modelo, Catalogo.tabla, tabla1.nombre AS nombre_tabla, Catalogo.pk_tabla, Catalogo.icono, Catalogo.titulo_nuevo, Catalogo.titulo_edicion, Catalogo.titulo_busqueda, Catalogo.msg_creado, Catalogo.msg_actualizado, Catalogo.pregunta_eliminar, Catalogo.msg_eliminado, Catalogo.msg_cambios, Catalogo.campos_busqueda FROM '.$this->tabla.' Catalogo '.$joins.$filtros.' limit :start,:limit';
		}else{
