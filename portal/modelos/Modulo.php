...
class ModuloModelo extends Modelo{	
	...
	function buscar($params){
		...
		if ( !empty($params['filtros']) ){
			foreach($params['filtros'] as $filtro){
				...
				if ( $filtro['dataKey']=='id' ) {
					$filtros .= ' Modulo.id like :id OR ';
				} 
				if ( $filtro['dataKey']=='nombre' ) {
					$filtros .= ' Modulo.nombre like :nombre OR ';
				} 
				if ( $filtro['dataKey']=='icono' ) {
					$filtros .= ' Modulo.icono like :icono OR ';
				} 
				if ( $filtro['dataKey']=='nombre_interno' ) {
					$filtros .= ' Modulo.nombre_interno like :nombre_interno OR ';
				} 
				if ( $filtro['dataKey']=='ruta_base' ) {
					$filtros .= ' Modulo.ruta_base like :ruta_base OR ';
				} 
				if ( $filtro['dataKey']=='orden' ) {
					$filtros .= ' Modulo.orden like :orden OR ';
				}			
			}
		...
		$sql = 'SELECT COUNT(*) as total FROM '.$this->tabla.' Modulo '.$joins.$filtros;				
		$sth = $pdo->prepare($sql);		
		if ( !empty($params['filtros']) ){
			foreach($params['filtros'] as $filtro){
		...
		if ($paginar){
			$limit=$params['limit'];
			$start=$params['start'];
			$sql = 'SELECT Modulo.id, Modulo.nombre, Modulo.icono, Modulo.nombre_interno, Modulo.ruta_base, Modulo.orden FROM '.$this->tabla.' Modulo '.$joins.$filtros.' limit :start,:limit';
		}else{
			$sql = 'SELECT Modulo.id, Modulo.nombre, Modulo.icono, Modulo.nombre_interno, Modulo.ruta_base, Modulo.orden FROM '.$this->tabla.' Modulo '.$joins.$filtros;
		}
	...
	function obtener( $llave ){		
		$sql = 'SELECT Modulo.id, Modulo.nombre, Modulo.icono, Modulo.nombre_interno, Modulo.ruta_base, Modulo.orden
 FROM system_modulos AS Modulo
  WHERE Modulo.id=:id';
		$pdo = $this->getConexion();
		$sth = $pdo->prepare($sql);
