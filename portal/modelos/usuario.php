...
class UsuarioModelo extends Modelo{	
	...
	function buscar($params){
		...
		if ($paginar){
			$limit=$params['limit'];
			$start=$params['start'];
			$sql = 'SELECT  FROM '.$this->tabla.' Usuario '.$joins.$filtros.' limit :start,:limit';
		}else{
			$sql = 'SELECT  FROM '.$this->tabla.' Usuario '.$joins.$filtros;
		}
	...
	function obtener( $llave ){		
		$sql = 'SELECT 
 FROM system_users AS Usuario
  WHERE Usuario.{CAMPOLLAVE}=:{CAMPOLLAVE}';
		$pdo = $this->getConexion();
		$sth = $pdo->prepare($sql);
		 $sth->BindValue(':{CAMPOLLAVE}',$llave ); 
		$exito = $sth->execute();
		if ( !$exito ){
			$error =  $this->getError( $sth );
