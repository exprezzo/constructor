var EdicionTipos_deduccion = function(){
	this.editado=false;
	this.tituloNuevo='Nuevo Tipo de Deducci贸n';
	this.saveAndClose=false;
	
	
	this.borrar=function(){		
		var r=confirm("Tipo de Deducci贸n Eliminada");
		if (r==true){
		  this.eliminar();
		}
	}
	...
	this.close=function(){
		...
		if (this.editado){
			var res=confirm('驴Guardar antes de salir?');
			if (res===true){
	...
	//agrega una clase al panel del contenido y a la pesta馻 relacionada.
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Tipo de Deducci贸n: ' +getValorCampo('descripcion'));
		}else{
