var DeduccionesDeNomina=function (){	
	
	this.configurarComboFk_TipoDeduccion=function(target){		
		...
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/tipos_deduccion/buscar',
			dataType:"json",
			type:'POST'
		});
	...
	this.init=function(config){
		...
		var me = this;
		$(this.tabId + "-dialog-confirm-eliminar-deduccion_nomina").wijdialog({
			autoOpen: false,
            captionButtons: {                  
				pin: { visible: false },
	...
	this.configurarGrid=function(targetSelector, articulos){		
		...
		gridElementos.bind('keydown', function(e) {
			...
			if(e.keyCode==46 && e.shiftKey){
				...
				$(me.tabId + "-dialog-confirm-eliminar-deduccion_nomina").wijdialog('open');
			}
		});
		...
		gridElementos.wijgrid({ beforeCellEdit: function(e, args) {
			...
			case "descripcion_fk_TipoDeduccion":
				var w,h;
				...
				me.configurarComboFk_TipoDeduccion(combo);						
			break;
				default:						
					var domCel = args.cell.tableCell();						
					var w,h;
