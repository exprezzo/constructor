var PercepcionesDeNomina=function (){	
	
	this.configurarComboFk_TipoPercepcion=function(target){		
		...
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/tipos_percepcion/buscar',
			dataType:"json",
			type:'POST'
		});
	...
	this.init=function(config){
		...
		var me = this;
		$(this.tabId + "-dialog-confirm-eliminar-percepcion_nomina").wijdialog({
			autoOpen: false,
            captionButtons: {                  
				pin: { visible: false },
	...
	this.configurarGrid=function(targetSelector, articulos){		
		...
		gridElementos.bind('keydown', function(e) {
			...
			if(e.keyCode==46 && e.shiftKey){
				...
				$(me.tabId + "-dialog-confirm-eliminar-percepcion_nomina").wijdialog('open');
			}
		});
		...
		gridElementos.wijgrid({
			...
			columns: [
				...
				{ dataKey: "descripcion_fk_TipoPercepcion", visible:true, headerText: "Tipo Percepcion" },
				{ dataKey: "fk_TipoPercepcion", visible:false, headerText: "Tipo Percepcion" },
		...
		gridElementos.wijgrid({ beforeCellEdit: function(e, args) {
			...
			case "descripcion_fk_TipoPercepcion":
				var w,h;
				...
				me.configurarComboFk_TipoPercepcion(combo);						
			break;
				default:						
					var domCel = args.cell.tableCell();						
					var w,h;
