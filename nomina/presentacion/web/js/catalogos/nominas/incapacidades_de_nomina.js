var IncapacidadesDeNomina=function (){	
	
	this.configurarComboFk_TipoIncapacidad=function(target){		
		...
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/tipos_incapacidad/buscar',
			dataType:"json",
			type:'POST'
		});
	...
	this.init=function(config){
		...
		var me = this;
		$(this.tabId + "-dialog-confirm-eliminar-incapacidad").wijdialog({
			autoOpen: false,
            captionButtons: {                  
				pin: { visible: false },
	...
	this.configurarGrid=function(targetSelector, articulos){		
		...
		gridElementos.bind('keydown', function(e) {
			...
			if(e.keyCode==46 && e.shiftKey){
				...
				$(me.tabId + "-dialog-confirm-eliminar-incapacidad").wijdialog('open');
			}
		});
		...
		gridElementos.wijgrid({ beforeCellEdit: function(e, args) {
			...
			case "descripcion_fk_TipoIncapacidad":
				var w,h;
				...
				me.configurarComboFk_TipoIncapacidad(combo);						
			break;
				default:						
					var domCel = args.cell.tableCell();						
					var w,h;
