var EdicionCertificados = function(){
	this.editado=false;
	this.tituloNuevo='Nuevo Certificado';
	this.saveAndClose=false;
	
	this.configurarComboFk_razon_social=function(){
		var me=this;
		
		$('select[name="fk_razon_social"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:false,	
		 ...
		 $('.contenedor_fk_razon_social input[role="textbox"]').bind('keypress', function(){			
			if (me.Fk_razon_socialEnAjax) return true;			
			me.setDSFk_razon_social();
			me.Fk_razon_socialEnAjax=true;
		 });
	};
		
		
	this.setDSFk_razon_social = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/certificados/buscarEmpresa',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		{name:'label', mapping:'nombre_comercial' }, 
		...
		$('select[name="fk_razon_social"]').wijcombobox('option','data',datasource);
	};
		
	
	this.borrar=function(){		
		var r=confirm("¿Eliminar Certificado?");
		if (r==true){
		  this.eliminar();
		}
	}
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Certificado: ' + getValorCampo('Certificado'));
		}else{
	...
	this.guardar=function(){
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_razon_social"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_razon_social"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_razon_social'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_razon_social'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_razon_social'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
...
this.configurarComboFk_razon_social();
	};
