var PdvsDeCliente=function (){	
	
	this.configurarComboFk_pais=function(target){		
		...
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/paises/buscar',
			dataType:"json",
			type:'POST'
		});
	...
	this.configurarComboFk_estado=function(target){		
		var tabId=this.tabId;
		var me=this;
		var fields=[										
		...
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/estados/buscar',
			dataType:"json",
			type:'POST'
		});
	...
	this.configurarComboFk_municipio=function(target){		
		var tabId=this.tabId;
		var me=this;
		var fields=[										
		...
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/municipios/buscar',
			dataType:"json",
			type:'POST'
		});
	...
	this.configurarComboFk_cliente=function(target){		
		var tabId=this.tabId;
		var me=this;
		var fields=[										
		...
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/clientes/buscar',
			dataType:"json",
			type:'POST'
		});
	...
	this.init=function(config){
		...
		var me = this;
		$(this.tabId + "-dialog-confirm-eliminar-punto_de_venta").wijdialog({
			autoOpen: false,
            captionButtons: {                  
				pin: { visible: false },
	...
	this.configurarGrid=function(targetSelector, articulos){		
		...
		gridElementos.bind('keydown', function(e) {
			...
			if(e.keyCode==46 && e.shiftKey){
				...
				$(me.tabId + "-dialog-confirm-eliminar-punto_de_venta").wijdialog('open');
			}
		});
		...
		gridElementos.wijgrid({ beforeCellEdit: function(e, args) {
			...
			case "nombre_fk_pais":
				...
				me.configurarComboFk_pais(combo);						
			break;
			case "nombre_fk_estado":
				var w,h;
				var domCel = args.cell.tableCell();
				w = $(domCel).width() ;
				h = $(domCel).height() ;
				...
				me.configurarComboFk_estado(combo);						
			break;
			case "nombre_fk_municipio":
				var w,h;
				var domCel = args.cell.tableCell();
				w = $(domCel).width() ;
				h = $(domCel).height() ;
				...
				me.configurarComboFk_municipio(combo);						
			break;
			case "nombre_fk_cliente":
				var w,h;
				var domCel = args.cell.tableCell();
				w = $(domCel).width() ;
				h = $(domCel).height() ;
				...
				me.configurarComboFk_cliente(combo);						
			break;
				default:						
					var domCel = args.cell.tableCell();						
					var w,h;
		...
		gridElementos.wijgrid({beforeCellUpdate:function(e, args) {
			...
			case "nombre_fk_pais":
				...
				if (me.pais!=undefined){
					var row=args.cell.row();					
					row.data.fk_pais = me.pais.value;					
			...
			case "nombre_fk_estado":
				...
				if (me.estado!=undefined){
					var row=args.cell.row();					
					row.data.fk_estado = me.estado.value;					
					gridElementos.wijgrid('ensureControl',true);					
				}
				break;
			case "nombre_fk_municipio":
				...
				if (me.municipio!=undefined){
					var row=args.cell.row();					
					row.data.fk_municipio = me.municipio.value;					
					gridElementos.wijgrid('ensureControl',true);					
				}
				break;
			case "nombre_fk_cliente":
				...
				if (me.cliente!=undefined){
					var row=args.cell.row();					
					row.data.fk_cliente = me.cliente.value;					
					gridElementos.wijgrid('ensureControl',true);					
				}
				break;
					default:						
