var EdicionClientes = function(){
	this.editado=false;
	this.tituloNuevo='Nuevo Cliente';
	...
	this.setDSFk_pais = function(){		
		...
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/clientes/buscarPais',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
	...
	this.setDSFk_estado = function(){		
		...
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/clientes/buscarEstado',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
	...
	this.setDSFk_municipio = function(){		
		...
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/clientes/buscarMunicipio',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Cliente: '+getValorCampo('nombre'));
		}else{
	...
	this.guardar=function(){
		...
		//-----------------------------------
		var datos=paramObj;
		$(tabId+' .tabla_pdvs').wijgrid('endEdit');
				var pdvs=$(tabId+' .tabla_pdvs').wijgrid('data');
				datos.pdvsDeCliente = pdvs;
		...
		$.ajax({
			...
			if ( resp.success == true	){
				...
				//--------------------
				var elementos=resp.datos.pdvsDeCliente;	

				var grid=$(me.tabId+" .tabla_pdvs");
