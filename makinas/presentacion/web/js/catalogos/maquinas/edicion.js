var EdicionMaquinas = function(){
	this.editado=false;
	this.tituloNuevo='Nueva Maquina';
	this.saveAndClose=false;
	
	this.configurarComboFk_owner=function(){
		var me=this;
		
		$('select[name="fk_owner"]').wijcombobox({			
			showTrigger: true,
			width:300,
			minLength:1,
			autoFilter:false,			
		 ...
		 $('.contenedor_fk_owner input[role="textbox"]').bind('focus', function(){			
			if (me.Fk_ownerEnAjax) return true;			
			me.setDSFk_owner();
			me.Fk_ownerEnAjax=true;
		 });
	};
		
		
	this.setDSFk_owner = function(){		
		
		var filtering=new Array();
		var proxy = new wijhttpproxy({
			url: kore.url_base+kore.modulo+'/maquinas/buscarOwner',
			dataType: "json", 
			type:"POST",
			data: {
				style: "full",
		...
		$('select[name="fk_owner"]').wijcombobox('option','data',datasource);
	};
	...
	this.actualizarTitulo=function(){
		...
		if (id>0){						
			$(tabId +' #titulo h1').html('Maquina: ' + getValorCampo('nombre'));
		}else{
	...
	this.guardar=function(){
		...
		//-----------------------------------		
		var selectedIndex = $('[name="fk_owner"]').wijcombobox('option','selectedIndex');  
		var selectedItem = $('[name="fk_owner"]').wijcombobox("option","data");		
		if (selectedIndex == -1){
			paramObj['fk_owner'] =0;
		}else{
			if (selectedItem.data == undefined ){
				paramObj['fk_owner'] =selectedItem[selectedIndex]['value'];
			}else{
				paramObj['fk_owner'] =selectedItem.data[selectedIndex]['id'];
			}
		}
		//-----------------------------------
