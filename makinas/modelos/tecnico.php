<?php
class tecnicoModelo extends Modelo{	
	var $tabla='makina_tecnico';
	var $pk='id';
	...
	function buscar($params){
		...
		if ( !empty($params['filtros']) ){
			foreach($params['filtros'] as $filtro){
				...
				if ( $filtro['dataKey']=='id' ) {
					$filtros .= ' tecnico.id like :id OR ';
				} 
				if ( $filtro['dataKey']=='nombre' ) {
					$filtros .= ' tecnico.nombre like :nombre OR ';
				} 
				if ( $filtro['dataKey']=='email' ) {
					$filtros .= ' tecnico.email like :email OR ';
				} 
				if ( $filtro['dataKey']=='datos_de_contacto' ) {
					$filtros .= ' tecnico.datos_de_contacto like :datos_de_contacto OR ';
				} 
				if ( $filtro['dataKey']=='calle' ) {
					$filtros .= ' tecnico.calle like :calle OR ';
				} 
				if ( $filtro['dataKey']=='numero_interior' ) {
					$filtros .= ' tecnico.numero_interior like :numero_interior OR ';
				} 
				if ( $filtro['dataKey']=='numero_exterior' ) {
					$filtros .= ' tecnico.numero_exterior like :numero_exterior OR ';
				} 
				if ( $filtro['dataKey']=='colonia' ) {
					$filtros .= ' tecnico.colonia like :colonia OR ';
				} 
				if ( $filtro['dataKey']=='localidad' ) {
					$filtros .= ' tecnico.localidad like :localidad OR ';
				} 
				if ( $filtro['dataKey']=='referencia' ) {
					$filtros .= ' tecnico.referencia like :referencia OR ';
				} 
				if ( $filtro['dataKey']=='fk_pais' ) {
					$filtros .= ' tecnico.fk_pais like :fk_pais OR ';
				} 
				if ( $filtro['dataKey']=='nombre_pais' ) {
					$filtros .= ' pais0.nombre like :nombre_pais OR ';
				} 
				if ( $filtro['dataKey']=='fk_estado' ) {
					$filtros .= ' tecnico.fk_estado like :fk_estado OR ';
				} 
				if ( $filtro['dataKey']=='nombre_estado' ) {
					$filtros .= ' estado1.nombre like :nombre_estado OR ';
				} 
				if ( $filtro['dataKey']=='fk_municipio' ) {
					$filtros .= ' tecnico.fk_municipio like :fk_municipio OR ';
				} 
		...
		$joins='
 LEFT JOIN system_ubicacion_paises AS pais0 ON pais0.id = tecnico.fk_pais
 LEFT JOIN system_ubicacion_estados AS estado1 ON estado1.id = tecnico.fk_estado
 LEFT JOIN system_ubicacion_municipios AS municipio2 ON municipio2.id = tecnico.fk_municipio';
						
		$sql = 'SELECT COUNT(*) as total FROM '.$this->tabla.' tecnico '.$joins.$filtros;				
		$sth = $pdo->prepare($sql);		
		if ( !empty($params['filtros']) ){
			foreach($params['filtros'] as $filtro){
		...
		if ($paginar){
			$limit=$params['limit'];
			$start=$params['start'];
			$sql = 'SELECT tecnico.id, tecnico.nombre, tecnico.email, tecnico.datos_de_contacto, tecnico.calle, tecnico.numero_interior, tecnico.numero_exterior, tecnico.colonia, tecnico.localidad, tecnico.referencia, tecnico.fk_pais, pais0.nombre AS nombre_fk_pais, tecnico.fk_estado, estado1.nombre AS nombre_fk_estado, tecnico.fk_municipio, municipio2.nombre AS nombre_fk_municipio FROM '.$this->tabla.' tecnico '.$joins.$filtros.' limit :start,:limit';
		}else{
			$sql = 'SELECT tecnico.id, tecnico.nombre, tecnico.email, tecnico.datos_de_contacto, tecnico.calle, tecnico.numero_interior, tecnico.numero_exterior, tecnico.colonia, tecnico.localidad, tecnico.referencia, tecnico.fk_pais, pais0.nombre AS nombre_fk_pais, tecnico.fk_estado, estado1.nombre AS nombre_fk_estado, tecnico.fk_municipio, municipio2.nombre AS nombre_fk_municipio FROM '.$this->tabla.' tecnico '.$joins.$filtros;
		}
	...
	function obtener( $llave ){		
		$sql = 'SELECT tecnico.id, tecnico.nombre, tecnico.email, tecnico.datos_de_contacto, tecnico.calle, tecnico.numero_interior, tecnico.numero_exterior, tecnico.colonia, tecnico.localidad, tecnico.referencia, tecnico.fk_pais, pais0.nombre AS nombre_fk_pais, tecnico.fk_estado, estado1.nombre AS nombre_fk_estado, tecnico.fk_municipio, municipio2.nombre AS nombre_fk_municipio
 FROM makina_tecnico AS tecnico
 LEFT JOIN system_ubicacion_paises AS pais0 ON pais0.id = tecnico.fk_pais
 LEFT JOIN system_ubicacion_estados AS estado1 ON estado1.id = tecnico.fk_estado
 LEFT JOIN system_ubicacion_municipios AS municipio2 ON municipio2.id = tecnico.fk_municipio
  WHERE tecnico.id=:id';
