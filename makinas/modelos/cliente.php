...
class clienteModelo extends Modelo{	
	...
	function buscar($params){
		...
		if ( !empty($params['filtros']) ){
			foreach($params['filtros'] as $filtro){
				...
				if ( $filtro['dataKey']=='id' ) {
					$filtros .= ' cliente.id like :id OR ';
				} 
				if ( $filtro['dataKey']=='nombre' ) {
					$filtros .= ' cliente.nombre like :nombre OR ';
				} 
				if ( $filtro['dataKey']=='email' ) {
					$filtros .= ' cliente.email like :email OR ';
				} 
				if ( $filtro['dataKey']=='datos_de_contacto' ) {
					$filtros .= ' cliente.datos_de_contacto like :datos_de_contacto OR ';
				} 
				if ( $filtro['dataKey']=='calle' ) {
					$filtros .= ' cliente.calle like :calle OR ';
				} 
				if ( $filtro['dataKey']=='numero_interior' ) {
					$filtros .= ' cliente.numero_interior like :numero_interior OR ';
				} 
				if ( $filtro['dataKey']=='numero_exterior' ) {
					$filtros .= ' cliente.numero_exterior like :numero_exterior OR ';
				} 
				if ( $filtro['dataKey']=='colonia' ) {
					$filtros .= ' cliente.colonia like :colonia OR ';
				} 
				if ( $filtro['dataKey']=='localidad' ) {
					$filtros .= ' cliente.localidad like :localidad OR ';
				} 
				if ( $filtro['dataKey']=='referencia' ) {
					$filtros .= ' cliente.referencia like :referencia OR ';
				} 
				if ( $filtro['dataKey']=='fk_pais' ) {
					$filtros .= ' cliente.fk_pais like :fk_pais OR ';
				} 
				if ( $filtro['dataKey']=='nombre_pais' ) {
					$filtros .= ' pais0.nombre like :nombre_pais OR ';
				} 
				if ( $filtro['dataKey']=='fk_estado' ) {
					$filtros .= ' cliente.fk_estado like :fk_estado OR ';
				} 
				if ( $filtro['dataKey']=='nombre_estado' ) {
					$filtros .= ' estado1.nombre like :nombre_estado OR ';
				} 
				if ( $filtro['dataKey']=='fk_municipio' ) {
					$filtros .= ' cliente.fk_municipio like :fk_municipio OR ';
				} 
		...
		$joins='
 LEFT JOIN system_ubicacion_paises AS pais0 ON pais0.id = cliente.fk_pais
 LEFT JOIN system_ubicacion_estados AS estado1 ON estado1.id = cliente.fk_estado
 LEFT JOIN system_ubicacion_municipios AS municipio2 ON municipio2.id = cliente.fk_municipio';
						
		$sql = 'SELECT COUNT(*) as total FROM '.$this->tabla.' cliente '.$joins.$filtros;				
		$sth = $pdo->prepare($sql);		
		if ( !empty($params['filtros']) ){
			foreach($params['filtros'] as $filtro){
		...
		if ($paginar){
			$limit=$params['limit'];
			$start=$params['start'];
			$sql = 'SELECT cliente.id, cliente.nombre, cliente.email, cliente.datos_de_contacto, cliente.calle, cliente.numero_interior, cliente.numero_exterior, cliente.colonia, cliente.localidad, cliente.referencia, cliente.fk_pais, pais0.nombre AS nombre_fk_pais, cliente.fk_estado, estado1.nombre AS nombre_fk_estado, cliente.fk_municipio, municipio2.nombre AS nombre_fk_municipio FROM '.$this->tabla.' cliente '.$joins.$filtros.' limit :start,:limit';
		}else{
			$sql = 'SELECT cliente.id, cliente.nombre, cliente.email, cliente.datos_de_contacto, cliente.calle, cliente.numero_interior, cliente.numero_exterior, cliente.colonia, cliente.localidad, cliente.referencia, cliente.fk_pais, pais0.nombre AS nombre_fk_pais, cliente.fk_estado, estado1.nombre AS nombre_fk_estado, cliente.fk_municipio, municipio2.nombre AS nombre_fk_municipio FROM '.$this->tabla.' cliente '.$joins.$filtros;
		}
	...
	function nuevo( $params ){
		$obj=array();
			...
			$obj['pdvsDeCliente']=array();
			
		return $obj;
	}
	function obtener( $llave ){		
		$sql = 'SELECT cliente.id, cliente.nombre, cliente.email, cliente.datos_de_contacto, cliente.calle, cliente.numero_interior, cliente.numero_exterior, cliente.colonia, cliente.localidad, cliente.referencia, cliente.fk_pais, pais0.nombre AS nombre_fk_pais, cliente.fk_estado, estado1.nombre AS nombre_fk_estado, cliente.fk_municipio, municipio2.nombre AS nombre_fk_municipio
 FROM makina_cliente AS cliente
 LEFT JOIN system_ubicacion_paises AS pais0 ON pais0.id = cliente.fk_pais
 LEFT JOIN system_ubicacion_estados AS estado1 ON estado1.id = cliente.fk_estado
 LEFT JOIN system_ubicacion_municipios AS municipio2 ON municipio2.id = cliente.fk_municipio
  WHERE cliente.id=:id';
		...
		if ( sizeof($modelos) > 1 ){			
			throw new Exception("El identificador está duplicado"); //TODO: agregar numero de error, crear una exception MiEscepcion
				...
				//----------------------------
				$conceptosMod=new punto_de_ventaModelo();
				$params=array(
					'filtros'=>array(
						array(
				...
				$pdvsDeCliente=$conceptosMod->buscar($params);				
				$modelos[0]['pdvsDeCliente'] =$pdvsDeCliente['datos'];
				//---------------------------
	...
	function guardar( $datos ){
		...
		foreach( $datos['pdvsDeCliente'] as $el ){
			if ( !empty($el['eliminado']) ){
				if ( !empty($el['id']) ){
					$res = $punto_de_ventaMod->eliminar( array('id'=>$el['id']) );
				...
				$res = $punto_de_ventaMod->guardar($el);
			 }
